...
const LoginCollectivite = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(apiUrl('/api/auth/login-collectivite'), {
        ...
        {loading ? (
          <div className="text-center">
            ...
            <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-xl">
              ...
              <div className="text-green-600 text-sm mt-1">Redirection vers votre dashboard...</div>
            </div>
          </div>
        ) : (
          <div className="bg-white/80 backdrop-blur-lg rounded-2xl border border-neutral-200/50 shadow-xl p-8">
            <div className="text-center mb-6">
              <h1 className="text-3xl font-display font-bold text-neutral-800">Connexion Collectivité</h1>
              <p className="text-neutral-600 mt-2">🏢 Accès administrateur</p>
            </div>
            ...
            <form onSubmit={submit} className="space-y-4">
              ...
              <button 
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-r from-primary-600 to-primary-700 text-white px-6 py-3 rounded-xl font-semibold hover:shadow-lg hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:transform-none"
              >
...
export default LoginCollectivite
